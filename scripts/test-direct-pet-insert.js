/**
 * Test direct PET contribution insertion
...
async function testDirectPetInsert() {
  console.log('🧪 Testing Direct PET Contribution Insertion...\n');

  try {
    // 1. Find a collection with PET materials
    console.log('1. Finding collection with PET materials...');
    ...
    // 2. Simulate the API logic
    console.log(`\n2. Simulating API logic for collection ${testCollectionId}...`);
    ...
    if (existing && existing.length > 0) {
      ...
      // Show existing contribution
      const { data: existingContrib, error: existingErr } = await supabase
        .from('green_scholar_transactions')
        .select('*')
        .eq('source_id', testCollectionId)
      ...
      if (!existingErr && existingContrib) {
        console.log('   📋 Existing contributions:');
        existingContrib.forEach((t, i) => {
    ...
    // 4. Test the Green Scholar Fund service logic
    console.log('\n4. Testing Green Scholar Fund service logic...');
    ...
    if (allError) {
      ...
      const donations = sum(allTransactions, 'donation') || sum(allTransactions, 'direct_donation');
      const disbursed = sum(allTransactions, 'distribution') || sum(allTransactions, 'expense');
      
      console.log('   📊 Green Scholar Fund Overview:');
      console.log(`     Total PET Revenue: R${petContrib || petDonation || 0}`);
      console.log(`     pet_contribution: R${petContrib}`);
      console.log(`     pet_donation: R${petDonation}`);
      console.log(`     Direct Donations: R${donations}`);
      console.log(`     Disbursed: R${disbursed}`);
      console.log(`     Remaining Balance: R${(petContrib || petDonation || 0) + donations - disbursed}`);
    }
...
// Run the test
testDirectPetInsert().catch(console.error);
