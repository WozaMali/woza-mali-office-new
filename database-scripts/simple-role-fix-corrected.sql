-- Simple fix for role_id type issues without changing column types
-- This approach works with existing column types and correct column names
...
WHERE table_name = 'users' 
  AND column_name IN ('role_id', 'id', 'township_id', 'department')
ORDER BY column_name;

-- Step 2: Create views that work with existing column types
-- View for team members with text casting
...
LEFT JOIN public.townships t ON u.township_id = t.id
...
BEGIN
    ...
    -- Insert user record with correct column names
...
BEGIN
    ...
    RAISE NOTICE 'The application should now work without type conversion issues.';
END $$;
