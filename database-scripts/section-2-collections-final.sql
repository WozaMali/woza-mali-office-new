-- ============================================================================
-- SECTION 2: COLLECTIONS MANAGEMENT SCHEMA - FINAL CLEAN VERSION
-- ============================================================================
-- This script completely overrides existing materials and collections
-- Updates all 3 apps: Main App, Collector App, Office App

-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ============================================================================
-- 1. CLEANUP EXISTING DATA AND POLICIES
-- ============================================================================
...
DROP TRIGGER IF EXISTS calculate_green_scholar_fund_trigger ON public.collections;

-- Drop existing views
DROP VIEW IF EXISTS public.collection_details CASCADE;
DROP VIEW IF EXISTS public.collector_stats CASCADE;
...
-- ============================================================================
-- 3. RECREATE COLLECTIONS TABLE
-- ============================================================================

-- Drop and recreate collections table
...
CREATE INDEX idx_collections_resident ON public.collections(resident_id);
CREATE INDEX idx_collections_collector ON public.collections(collector_id);
CREATE INDEX idx_collections_area ON public.collections(area_id);
CREATE INDEX idx_collections_material ON public.collections(material_id);
...
-- ============================================================================
-- 8. INSERT FRESH MATERIALS DATA
-- ============================================================================

-- Insert materials with correct pricing
...
CREATE VIEW public.collector_stats AS
SELECT 
    c.collector_id,
...
SELECT '✅ Section 2: Collections Management Schema - FINAL CLEAN VERSION COMPLETE!' as status;
SELECT '📊 Materials table recreated with fresh data' as info;
SELECT '📦 Collections table recreated with RLS policies' as info;
SELECT '🌱 Green Scholar Fund tracking enabled' as info;
SELECT '👁️ Views created: collection_details, collector_stats' as info;
SELECT '🔒 RLS policies applied for security' as info;
SELECT '📈 Performance indexes created' as info;

-- Show materials count
SELECT 'Materials inserted:' as info, COUNT(*) as count FROM public.materials;
