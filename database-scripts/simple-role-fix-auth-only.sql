...
-- This approach works with the actual users table structure and avoids auth user creation
...
-- Step 3: Create user profile function that works with existing auth users
...
BEGIN
    ...
    -- Insert user profile record (user must already exist in auth.users)
    INSERT INTO public.users (
        ...
        'active',
        v_emp_number,
        NULL, -- Set township_id to NULL to avoid UUID conversion issues
        NOW(),
        NOW()
    );
...
-- Step 5: Test the function (this will fail because the user doesn't exist in auth.users)
SELECT 'Testing create_user_profile...' as test;
-- This will fail because the user doesn't exist in auth.users, which is expected
...
BEGIN
    ...
    RAISE NOTICE '- create_user_profile() - User profile creation function';
    RAISE NOTICE 'The API route will handle auth user creation first, then call this function.';
END $$;
